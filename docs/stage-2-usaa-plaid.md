...
- Refresh household data reads saved backend data. The deployed **Sync bank data** makes user-initiated Balance/Transactions calls; the interface update also runs those calls when the user checks safe-to-spend. The authorized [bank sync controls release](bank-sync-controls.md) moves normal Sync to the dashboard and adds an explicit Trial-included Transactions Refresh request in Settings, followed by bounded transaction imports. It adds no webhook, background worker, or periodic refresh. Complete bank history, reviewed spending, and explicit reconciliation remain required even after a successful sync.
